...
const getInitialState = (): FormDataType => ({
  ...
  isAIUOpted: false,
  historyAIUOptedSS: "",
  historySamples: [{ cds: "", lp: "", debug: "" }],
  historyOverride: false,
  historyAttr: [],
  historyUAsPrice: [],
  historyUAsAvail: [],
  coverageImproved: "waiting",
  coverageSS: "",
  outputFormat: "text",
});

const AVAILABLE_USER_AGENTS = [
  "desktop",
  "webkit_desktop",
  "mobile",
  "webkit_mobile",
...
export default function App() {
  ...
  const [theme, setTheme] = useState<"dark" | "light">(
    () =>
      (localStorage.getItem("buganizer_theme") as "dark" | "light") || "dark",
  );
  ...
  const handleGenerate = useCallback(() => {
    if (
      !formData.name ||
      formData.attribute.length === 0 ||
      !formData.gearloose
    ) {
      showToast("Please fill Global Fields (Name, Attribute, Main Gearloose)");
      return null;
    }
    if (formData.activeScenarios.length === 0) {
      showToast("Please select at least one scenario block");
      return null;
    }
    ...
    if (
      ...
      if (!formData.historyOverride) {
        if (isPrice && formData.userAgentsPrice.length === 0) {
          showToast("Select User Agents for Price!");
          return null;
        }
        if (isAvail && formData.userAgentsAvail.length === 0) {
          showToast("Select User Agents for Availability!");
          return null;
        }
      }
    }
    ...
    if (!isValidLink(formData.gearloose)) {
      showToast("Invalid Gearloose Link!");
      return null;
    }

    if (formData.activeScenarios.includes("overrule")) {
      if (formData.overruleType === "dt") {
        if (isPrice && !isValidLink(formData.mismatchPriceSS)) {
          showToast("Invalid Price Mismatch Link!");
          return null;
        }
        if (isAvail && !isValidLink(formData.mismatchAvailSS)) {
          showToast("Invalid Availability Mismatch Link!");
          return null;
        }
      }
      if (formData.overruleType === "or") {
        if (isPrice && !isValidLink(formData.dashboardPriceSS)) {
          showToast("Invalid Price Dashboard Link!");
          return null;
        }
        if (isAvail && !isValidLink(formData.dashboardAvailSS)) {
          showToast("Invalid Availability Dashboard Link!");
          return null;
        }
        for (let issue of formData.orIssues) {
          if (!isValidLink(issue.mismatchSS)) {
            showToast("Invalid Mismatch SS in Issues!");
            return null;
          }
        }
      }
    ...
    if (formData.activeScenarios.includes("history")) {
      if (!formData.historyCondition) {
        showToast("Please select a History Condition!");
        return null;
      }
      if (!isValidLink(formData.historyReasonSS)) {
        showToast("Invalid Reason SS Link!");
        return null;
      }
      if (formData.isAIUOpted && !isValidLink(formData.historyAIUOptedSS)) {
        showToast("Invalid AIU Opted SS Link!");
        return null;
      }
    }
  ...
  const handleReset = () => {
    setFormData((prev) => ({
      ...
      userAgentsPrice: prev.userAgentsPrice,
      userAgentsAvail: prev.userAgentsAvail,
    }));
    setOutput("");
  ...
  const handleToggleScenario = (scenario: string) => {
    setFormData((prev) => {
      ...
      if (
        scenario === "history" &&
        prev.activeScenarios.includes("overrule") &&
        prev.overruleType === "or"
      ) {
        showToast(
          "ℹ️ Switched Overrule to DT (OR is incompatible with History)",
        );
        return {
          ...prev,
          activeScenarios: [...newActive, scenario],
          overruleType: "dt",
        };
      }
  ...
  const toggleArrayItem = (field: keyof FormDataType, item: string) => {
    setFormData((prev) => {
      if (
        field === "userAgentsPrice" ||
        field === "userAgentsAvail" ||
        field === "historyUAsPrice" ||
        field === "historyUAsAvail"
      ) {
        ...
        if (current.includes(item))
          return { ...prev, [field]: current.filter((a) => a !== item) };
        ...
        if (exists)
          return {
            ...prev,
            [field]: (prev[field] as string[]).filter((a) => a !== item),
          };
  ...
  const addArrayItem = (
    field: "orIssues" | "clSamples" | "historySamples",
    defaultObj: any,
  ...
  const activeHistoryAttrs =
    isHistoryActive &&
    isOverruleActive &&
    formData.historyOverride &&
    formData.historyAttr.length
  ...
  return (
    <div className="app-wrapper">
      ...
      <div className="main-layout">
        <div className="left-panel">
          <div className="card border-blue">
            ...
            <div className="input-group">
              ...
              <div className="segmented-control">
                <button
                  className={`segment-btn ${isPrice ? "active-segment" : ""}`}
                  onClick={() => toggleArrayItem("attribute", "price")}
                >
                  Price
                </button>
                <button
                  className={`segment-btn ${isAvail ? "active-segment" : ""}`}
                  onClick={() => toggleArrayItem("attribute", "availability")}
                >
                  Availability
                </button>
              </div>
            ...
            {isPrice && (
              <div className="input-group">
                ...
                >
                  <button
                    className={`segment-btn ${formData.userAgentsPrice.includes("all") ? "active-segment" : ""}`}
                    onClick={() => toggleArrayItem("userAgentsPrice", "all")}
                    style={{
                      gridColumn: "span 2",
                      border: "1px solid var(--input-border)",
                      background: formData.userAgentsPrice.includes("all")
                        ? "var(--segment-active-bg)"
                        : "var(--input-bg)",
                    }}
                  >
                    All User Agents
                  ...
                  {AVAILABLE_USER_AGENTS.map((ua) => (
                    <button
                      key={ua}
                      className={`segment-btn ${formData.userAgentsPrice.includes(ua) ? "active-segment" : ""}`}
                      onClick={() => toggleArrayItem("userAgentsPrice", ua)}
                      style={{
                        border: "1px solid var(--input-border)",
                        background: formData.userAgentsPrice.includes(ua)
                          ? "var(--segment-active-bg)"
                          : "var(--input-bg)",
                      }}
                    >
            ...
            {isAvail && (
              <div className="input-group">
                ...
                >
                  <button
                    className={`segment-btn ${formData.userAgentsAvail.includes("all") ? "active-segment" : ""}`}
                    onClick={() => toggleArrayItem("userAgentsAvail", "all")}
                    style={{
                      gridColumn: "span 2",
                      border: "1px solid var(--input-border)",
                      background: formData.userAgentsAvail.includes("all")
                        ? "var(--segment-active-bg)"
                        : "var(--input-bg)",
                    }}
                  >
                  ...
                  {AVAILABLE_USER_AGENTS.map((ua) => (
                    <button
                      key={ua}
                      className={`segment-btn ${formData.userAgentsAvail.includes(ua) ? "active-segment" : ""}`}
                      onClick={() => toggleArrayItem("userAgentsAvail", ua)}
                      style={{
                        border: "1px solid var(--input-border)",
                        background: formData.userAgentsAvail.includes(ua)
                          ? "var(--segment-active-bg)"
                          : "var(--input-bg)",
                      }}
                    >
          ...
          <div className="card">
            ...
            <div className="button-group-row">
              <button
                ...
                onClick={() => handleToggleScenario("overrule")}
              >
                + Overrule
              </button>
              <button
                ...
                onClick={() => handleToggleScenario("history")}
              >
                + History Mismatch
              </button>
              <button
                ...
                onClick={() => handleToggleScenario("cl")}
              >
                + CL Creation
              </button>
            </div>
            ...
            <button
              ...
              onClick={() => handleToggleScenario("coverage")}
            >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (formData.activeScenarios.includes("history")) {
                      handleChange(
                        "activeScenarios",
                        formData.activeScenarios.filter((s) => s !== "history"),
                      );
                      showToast(
                        "ℹ️ History Mismatch disabled (Incompatible with OR Comment)",
                      );
                    }
                  }}
                >
                ...
                <>
                  {isPrice && (
                    <div className="input-group">
                      ...
                      <input
                        placeholder="https://..."
                        value={formData.mismatchPriceSS}
                        onChange={(e) =>
                          handleChange("mismatchPriceSS", e.target.value)
                        }
                      />
                    </div>
                  )}
                  {isAvail && (
                    <div className="input-group">
                      ...
                      <input
                        placeholder="https://..."
                        value={formData.mismatchAvailSS}
                        onChange={(e) =>
                          handleChange("mismatchAvailSS", e.target.value)
                        }
                      />
                    </div>
                  )}
                  <div className="input-group" style={{ marginTop: "8px" }}>
                    ...
                    <input
                      ...
                      onChange={(e) => handleChange("bugLink", e.target.value)}
                    />
                  </div>
                </>
              )}
                ...
                <>
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChange("extractor", e.target.value)
                      }
                    />
                  </div>
                  <div className="divider-line"></div>
                  ...
                  {formData.orIssues.map((issue, i) => (
                    <div key={i} className="sample-block">
                      <div className="sample-header">
                        ...
                        {formData.orIssues.length > 1 && (
                          <button
                            ...
                            onClick={() => removeArrayItem("orIssues", i)}
                          >
                            Remove
                          </button>
                        )}
                      ...
                      {isPrice && isAvail && (
                        ...
                        >
                          <button
                            className={`segment-btn ${issue.attribute === "price" ? "active-segment" : ""}`}
                            onClick={() =>
                              updateArrayItem(
                                "orIssues",
                                i,
                          ...
                          <button
                            className={`segment-btn ${issue.attribute === "availability" ? "active-segment" : ""}`}
                            onClick={() =>
                              updateArrayItem(
                                "orIssues",
                                i,
                      ...
                      />
                      <input
                        placeholder={`Mismatches SS ${issue.attribute ? `(${issue.attribute})` : isPrice ? "(price)" : isAvail ? "(availability)" : ""} *`}
                        value={issue.mismatchSS}
                        onChange={(e) =>
                          updateArrayItem(
                            "orIssues",
                            i,
                  ...
                  {isPrice && (
                    <div className="input-group">
                      ...
                      <input
                        placeholder="https://..."
                        value={formData.dashboardPriceSS}
                        onChange={(e) =>
                          handleChange("dashboardPriceSS", e.target.value)
                        }
                      />
                    </div>
                  )}
                  {isAvail && (
                    <div className="input-group">
                      ...
                      <input
                        placeholder="https://..."
                        value={formData.dashboardAvailSS}
                        onChange={(e) =>
                          handleChange("dashboardAvailSS", e.target.value)
                        }
                      />
                    </div>
                  )}
          ...
          {formData.activeScenarios.includes("history") && (
            <div className="card highlight-card">
                ...
                <>
                  <div className="input-group" style={{ marginBottom: "12px" }}>
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={formData.historyOverride}
                        onChange={(e) =>
                          handleChange("historyOverride", e.target.checked)
                        }
                        style={{
                          width: "16px",
                          height: "16px",
                  ...
                  {formData.historyOverride && (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="segmented-control">
                          <button
                            className={`segment-btn ${formData.historyAttr.includes("price") ? "active-segment" : ""}`}
                            onClick={() =>
                              toggleArrayItem("historyAttr", "price")
                            }
                          >
                            Price
                          </button>
                          <button
                            className={`segment-btn ${formData.historyAttr.includes("availability") ? "active-segment" : ""}`}
                            onClick={() =>
                              toggleArrayItem("historyAttr", "availability")
                            }
                          >
                            Availability
                          </button>
                      ...
                      {isHistPrice && (
                        <div className="input-group">
                          ...
                          >
                            <button
                              className={`segment-btn ${formData.historyUAsPrice.includes("all") ? "active-segment" : ""}`}
                              onClick={() =>
                                toggleArrayItem("historyUAsPrice", "all")
                              }
                              style={{
                                gridColumn: "span 2",
                                border: "1px solid var(--input-border)",
                                background: formData.historyUAsPrice.includes(
                                  "all",
                                )
                                  ? "var(--segment-active-bg)"
                                  : "var(--input-bg)",
                            ...
                            {AVAILABLE_USER_AGENTS.map((ua) => (
                              <button
                                key={ua}
                                className={`segment-btn ${formData.historyUAsPrice.includes(ua) ? "active-segment" : ""}`}
                                onClick={() =>
                                  toggleArrayItem("historyUAsPrice", ua)
                                }
                                style={{
                                  border: "1px solid var(--input-border)",
                                  background: formData.historyUAsPrice.includes(
                      ...
                      {isHistAvail && (
                        <div className="input-group">
                          ...
                          >
                            <button
                              className={`segment-btn ${formData.historyUAsAvail.includes("all") ? "active-segment" : ""}`}
                              onClick={() =>
                                toggleArrayItem("historyUAsAvail", "all")
                              }
                              style={{
                                gridColumn: "span 2",
                                border: "1px solid var(--input-border)",
                                background: formData.historyUAsAvail.includes(
                                  "all",
                                )
                                  ? "var(--segment-active-bg)"
                                  : "var(--input-bg)",
                            ...
                            {AVAILABLE_USER_AGENTS.map((ua) => (
                              <button
                                key={ua}
                                className={`segment-btn ${formData.historyUAsAvail.includes(ua) ? "active-segment" : ""}`}
                                onClick={() =>
                                  toggleArrayItem("historyUAsAvail", ua)
                                }
                                style={{
                                  border: "1px solid var(--input-border)",
                                  background: formData.historyUAsAvail.includes(
                ...
                {isHistPrice || isHistAvail ? (
                  ...
                  >
                    {isHistPrice && (
                      <>
                        <button
                          className={`segment-btn ${formData.historyCondition === "BASE_INSTEAD_OF_SALE_THRESHOLD_REACHED" ? "active-segment" : ""}`}
                          onClick={() =>
                            handleChange(
                              "historyCondition",
                        ...
                        <button
                          className={`segment-btn ${formData.historyCondition === "BASE_PRICE_DISTRUST_THRESHOLD_REACHED" ? "active-segment" : ""}`}
                          onClick={() =>
                            handleChange(
                              "historyCondition",
                    ...
                    {isHistAvail && (
                      <button
                        className={`segment-btn ${formData.historyCondition === "AVAILABILITY_DISTRUST_THRESHOLD_REACHED" ? "active-segment" : ""}`}
                        onClick={() =>
                          handleChange(
                            "historyCondition",
                  ...
                  >
                    Please select an Attribute above!
                  </div>
                )}
              </div>
              ...
              <div className="input-group">
                ...
                <input
                  placeholder="https://..."
                  value={formData.historyReasonSS}
                  onChange={(e) =>
                    handleChange("historyReasonSS", e.target.value)
                  }
                />
              </div>
              <div
              ...
              >
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("isAIUOpted", e.target.checked)
                    }
                    style={{
                      width: "18px",
                      height: "18px",
              ...
              {formData.isAIUOpted && (
                <div className="input-group animate-slide-down">
                  ...
                  <input
                    placeholder="https://..."
                    value={formData.historyAIUOptedSS}
                    onChange={(e) =>
                      handleChange("historyAIUOptedSS", e.target.value)
                    }
                  />
                </div>
              )}
                ...
                <div key={i} className="sample-block">
                  <div className="sample-header">
                    ...
                    {formData.historySamples.length > 1 && (
                      <button
                        className="danger-text"
                        onClick={() => removeArrayItem("historySamples", i)}
                      >
                        Remove
                      </button>
                    )}
                  ...
                  <input
                    ...
                    value={sample.lp}
                    onChange={(e) =>
                      updateArrayItem("historySamples", i, "lp", e.target.value)
                    }
                  />
                  <input
              ...
              <button
                className="secondary outline-btn"
                onClick={() =>
                  addArrayItem("historySamples", { cds: "", lp: "", debug: "" })
                }
              >
                + Add Sample
              </button>
                ...
                <div key={i} className="sample-block">
                  <div className="sample-header">
                    ...
                    {formData.clSamples.length > 1 && (
                      <button
                        className="danger-text"
                        onClick={() => removeArrayItem("clSamples", i)}
                      >
                        Remove
                      </button>
                    )}
                  ...
                  <input
                    ...
                    value={sample.cds}
                    onChange={(e) =>
                      updateArrayItem("clSamples", i, "cds", e.target.value)
                    }
                  />
                  <input
                    placeholder="LP Link *"
                    value={sample.lp}
                    onChange={(e) =>
                      updateArrayItem("clSamples", i, "lp", e.target.value)
                    }
                  />
                  <input
                    placeholder="Debug Link *"
                    value={sample.debug}
                    onChange={(e) =>
                      updateArrayItem("clSamples", i, "debug", e.target.value)
                    }
                  />
                  <input
                    ...
                    value={sample.rating}
                    onChange={(e) =>
                      updateArrayItem("clSamples", i, "rating", e.target.value)
                    }
                  />
                  <input
          ...
          {formData.activeScenarios.includes("coverage") && (
            <div className="card highlight-card">
              ...
              <div className="input-group">
                ...
                <input
                  placeholder="https://..."
                  value={formData.coverageSS}
                  onChange={(e) => handleChange("coverageSS", e.target.value)}
                />
              </div>
        ...
        <div className="right-panel">
          <div className="tabs-container">
            <button
              className={`tab-btn ${activeTab === "output" ? "active-tab" : ""}`}
          ...
          {activeTab === "output" && (
            <div className="card sticky-output">
              ...
              <div className="hint-text">
                Press <b>Ctrl + Enter</b> to Generate & Copy instantly!
              </div>
              <div className="actions">
                ...
                <button className="danger-action" onClick={handleReset}>
                  Next Bug
                </button>
              </div>
            ...
            <div className="card sticky-output history-card">
              ...
              {history.length === 0 ? (
                <div className="empty-state">
                  No history yet. Generate a comment first!
                </div>
              ) : (
                        ...
                        <div className="accordion-body">
                          ...
                          >
                            ...
                            <button
                              ...
                              style={{ flex: 0, padding: "12px 20px" }}
      ...
      >
        🟢 SYS_STATUS: OFFLINE_SECURE | ENGINE: REACT_TS_X64 | BUILD_HASH:{" "}
